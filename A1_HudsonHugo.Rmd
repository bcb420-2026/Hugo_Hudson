...
# print(paste("Number of genes passing correction", length(model_results$adj.P.Val < 0.05)))
```
...
The top 100 hits based on p-value for differential expression are visualized in Figure 5, with the left half of columns pertaining to the control group and the right half mutation-induced. Based on the plot these 100 genes have two distinct groups with opposing trends, as a subset of the control group has up-regulated expression and the remaining subset is down regulated. When compared to the mutation-induced group the subsets flip, where down regulated genes become up-regulated and vice versa. There are 20,475 genes that pass p-value correction since this visualization contains only 100 of the genes this trend could represent a larger fraction of the passing genes.  If using the highest log fold change candidates for evaluating changes in expression, similar trends are observed between the groups yet with attenuated signal as more of the genes are populated around 0 z-scored expression.
